...
class ReportController extends Controller
{
    ...
    public function getTrendingProducts(Request $request)
    {
        ...
        // }
        $date_range = $request->input('datetimes');
        
        if (!empty($date_range)) {
            $date_range_array = explode('~', $date_range);
            $filters['start_date'] = date('Y-m-d H:i:s', strtotime($date_range_array[0]));
            $filters['end_date'] = date('Y-m-d H:i:s', strtotime($date_range_array[1]));
        }
        $date=date('Y-m-d');
        if ($request->input('date')) {
            $filters['date'] = date('Y-m-d', strtotime($request->input('date')));
            $date=$filters['date'];
        }else{
            $filters['date'] = $date;
        }
        ...
        $end_time=date('21:00');
        if ($request->input('location_id')) {
            $query_work_hours=DB::table('tbl_jam_kerja')->where('location_id', $request->input('location_id'))->first();
            $start_time=($query_work_hours != null ? $query_work_hours->jam_datang : date('09:00'));
            $end_time=($query_work_hours != null ? $query_work_hours->jam_pulang : date('21:00'));
        }

        $jam_datang=explode(':', $start_time);
        $jam_pulang=explode(':', $end_time);
        $data_detail=array();
        $j=0;
        $label_details = [];
        $temp=array();
        $null_data=array();
        for ($i=$jam_datang[0]; $i <= $jam_pulang[0]; $i++) { 
            $jam=$filters['date'].' '.$i;
            foreach ($products as $key => $value) {
                $get_count = $this->productUtil->countProductTrending($value->product_id, $business_id, $jam, $filters);
                $data_detail[$j]['products'][$key]['name']=$value->product;
                $data_detail[$j]['products'][$key]['detail']=$get_count;
                $temp[$key]['name']=$value->product;
                $temp[$key]['data'][$j]=($get_count != null ? $get_count->total_unit_sold : 0);
            }
            $null_data['name']='-';
            $null_data['data'][$j]=0;
            $label_details []= $i.":00:00";
            $data_detail[$j]['jam']=$i.":00:00";
            $j++;
        }
        $currency = Currency::where('id', request()->session()->get('business.currency_id'))->first();

        $trending = Charts::multi('line', 'highcharts')
                            ->title(' ')
                            ->template('material')
                            ->labels($label_details)
                            ->elementLabel('Total Terjual ('.date('d/m/Y', strtotime($date)).')');
        if (!empty($temp)) {
            foreach ($temp as $value) {
                $trending->dataset($value['name'], $value['data']);
            }
        }else{
            $trending->dataset($null_data['name'], $null_data['data']);
        }
        ...
        return view('report.trending_products')
                    ->with(compact('chart', 'categories', 'brands', 'units', 'business_locations', 'trending', 'date'));
    }

    public function getTrendingProducts2(Request $request)
    {
        if (!auth()->user()->can('trending_product_report.view')) {
            abort(403, 'Unauthorized action.');
        }
        ...
        $products = $this->productUtil->trendingProducts($business_id, $filters);
        ...
        return view('report.trending_product_filter')
        ...
        if ($request->ajax()) {
            $registers = CashRegister::join(
                ...
                ->select(
                    ...
                    DB::raw("ifnull((SELECT SUM(amount) from cash_register_transactions where transaction_type='sell' AND cash_register_transactions.cash_register_id=cash_registers.id), 0) as total")
                );
            ...
            return Datatables::of($registers)
                ...
                ->rawColumns(['action', 'user_name', 'closing_amount', 'total'])
                ->make(true);
        }
    ...
    public function getproductSellReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionSellLine::join(
                ...
                ->where('transaction_sell_lines.is_item_paket', 0)
                ->select(
                    'p.name as product_name',
                    'p.type as product_type',
                    'pv.name as product_variation',
                    ...
                    DB::raw('COALESCE((SELECT COUNT(id) from transaction_sell_lines tsl1 where transaction_id=t.id),0) as total_produk'),
                    't.discount_amount as da',
                    't.discount_type as dt',
                    't.total_before_tax',
            ...
            return Datatables::of($query)
                ...
                ->editColumn('sell_qty', function ($row) {
                 ...
                 ->editColumn('subtotal', function ($row) {
                     if ($row->da != 0) {
                        $discount=($row->dt == 'percentage' ? $row->total_before_tax * ($row->da/100) : $row->da) / $row->total_produk;
                        $subtotal=$row->quantity * ($row->unit_price - $discount);
    ...
    public function getproductSellAvgReport(Request $request)
    {
        if (!auth()->user()->can('purchase_n_sell_report.view')) {
            abort(403, 'Unauthorized action.');
        }
    ...
    public function getRekapTrxMember(Request $request){
        $date=date('Y-m');
        $business_id = request()->session()->get('user.business_id');
        $user_id = request()->session()->get('user.id');
        $user = User::where('id', $user_id)->first();
        ...
        if (request()->ajax()) {
            ...
            $data=Contact::select('contacts.*', DB::raw('COALESCE((SELECT COUNT(t.contact_id) FROM transactions t WHERE t.transaction_date like "'.$date.'%" AND t.contact_id=contacts.id '.($location_id == 0 ? '' : 'AND t.location_id='.$location_id ).'), 0) AS total_trx'))
                          ->whereNotNull('contacts.customer_group_id')
                          ->get();
            foreach ($data as $key => $value) {
                if ($value->total_trx == 0) {
                    unset($data[$key]);
                }
            }
        ...
        $month=json_encode(explode('-', $date));
        return view('report.trx_member')->with(compact('month', 'location_id', 'business_locations'));
    }
    public function getReportFinance(Request $request){
        $user_id = request()->session()->get('user.id');
        $user = User::where('id', $user_id)->first();   
        $pengeluaran=DB::table('tbl_pengeluaran')->select('tbl_pengeluaran.*', 'users.location_id')->join('users', 'tbl_pengeluaran.user_id', 'users.id')->where('tipe', 'pengeluaran')->orderby('tbl_pengeluaran.id')->get();
        // return($pengeluaran);
        foreach($pengeluaran as $key => $value){
            // $notes=null;
            // if(strpos(strtolower($value->deskripsi_pengeluaran), 'token') !== false){
            //     $notes='Listrik';
            // }elseif(strpos(strtolower($value->deskripsi_pengeluaran), 'galon') !== false || strpos(strtolower($value->deskripsi_pengeluaran), 'air') !== false || strpos(strtolower($value->deskripsi_pengeluaran), 'aqua') !== false){
            //     $notes='Biaya Air';
            // }elseif(strpos(strtolower($value->deskripsi_pengeluaran), 'laundry') !== false){
            //     $notes='Kebutuhan Salon';
            // }else{
            //     $notes='Pengeluaran Lain-Lain';
            // }
            // $query=DB::table('tbl_pengeluaran')->where('id', $value->id)->update(array('notes' => $notes));
            $akun=DB::table('tbl_akun')->where('nama_akun', $value->notes)->first();
        }
        exit;
        $location_id=0;
        $date=date('Y-m');
        ...
        for ($i=1; $i <= $jumlah_hari ; $i++) { 
            ...
            $day=$date.'-'.$hari;
             ...
             $transactions=DB::select('SELECT COALESCE((SELECT SUM(amount) FROM transaction_payments JOIN transactions ON transactions.id=transaction_payments.transaction_id WHERE paid_on like "'.$day.'%" AND method="cash" '.($location_id != 0 ? 'AND transactions.location_id="'.$location_id.'"' : '').'), 0) AS cash, COALESCE((SELECT SUM(amount) FROM transaction_payments JOIN transactions ON transactions.id=transaction_payments.transaction_id WHERE paid_on like "'.$day.'%" AND method="card" '.($location_id != 0 ? 'AND transactions.location_id="'.$location_id.'"' : '').'), 0) AS card, COALESCE((SELECT SUM(amount) FROM transaction_payments JOIN transactions ON transactions.id=transaction_payments.transaction_id WHERE paid_on like "'.$day.'%" AND method="ovo" '.($location_id != 0 ? 'AND transactions.location_id="'.$location_id.'"' : '').'), 0) AS ovo');
            $query_pengeluaran=DB::table('tbl_pengeluaran')
                        ->select('tbl_pengeluaran.total', 'tbl_pengeluaran.deskripsi_pengeluaran', 'tbl_pengeluaran.notes')
            ...
            $query=DB::table('tbl_pegawai')
                        ->select('tbl_pegawai.nama_pegawai', DB::raw('COALESCE((SELECT SUM(tp.total) FROM tbl_pengeluaran tp WHERE tp.tipe="kasbon" AND tp.tanggal="'.$day.'" AND tp.id_pegawai=tbl_pegawai.id_pegawai), 0) AS total_kasbon'));
            if ($location_id != 0) {
                $query->where('tbl_pegawai.location_id', $location_id);
            }
            $kasbon=$query->get();
            $data_kasbon[$j]['tanggal']=$day;
            $data_kasbon[$j]['kasbon']=$kasbon;
            $data_kasbon[$j]['pengeluaran']=$pengeluaran;
            $data_kasbon[$j]['transactions']=$transactions;
            $j++;
        }
        // return $data_kasbon;
        // exit;
        ...
        $business_locations[0]='Semua Lokasi';
        ...
        $gaji_report=$query_gaji->get();
        
        $query_adjusment=Transaction::where('transactions.type', 'stock_adjustment')
                                      ->select('products.name', 'product_id')
                                      ->join('stock_adjustment_lines', 'stock_adjustment_lines.transaction_id', '=', 'transactions.id')
                                      ->join('products', 'stock_adjustment_lines.product_id', '=', 'products.id')
                                      ->where('transaction_date', 'like', $date.'%');                                      
        if ($request->input('location_id')) {
            $query_adjusment->where('transactions.location_id', $request->input('location_id'));
        }
        $stock_adjustment=$query_adjusment->groupBy('product_id', 'transactions.location_id')
                                  ->get();
        foreach ($stock_adjustment as $key => $value) {
            $count=$this->countStockAdjustment($value->product_id, $date, $location_id);
            $value->total=$count;
        }
        
        return view('report.finance_report')->with(compact('month', 'location_id', 'business_locations', 'data_kasbon', 'gaji_report', 'stock_adjustment'));
    }
    private function countStockAdjustment($id, $date, $location_id){
        $query_adjusment=Transaction::where('transactions.type', 'stock_adjustment')
                                      ->select('products.name', 'product_id', 'quantity', 'unit_price')
                                      ->join('stock_adjustment_lines', 'stock_adjustment_lines.transaction_id', '=', 'transactions.id')
                                      ->join('products', 'stock_adjustment_lines.product_id', '=', 'products.id')
                                      ->where('stock_adjustment_lines.product_id', $id)
                                      ->where('transaction_date', 'like', $date.'%');                                      
        if ($location_id != 0) {
            $query_adjusment->where('transactions.location_id', $location_id);
        }
        $sa=$query_adjusment->get();
        $total=0;
    ...
    public function getRekapPromo(Request $request){
        $date=date('Y-m');
        $business_id = request()->session()->get('user.business_id');
        $user_id = request()->session()->get('user.id');
        $user = User::where('id', $user_id)->first();
        ...
        if (request()->ajax()) {
            ...
            $query=Promo::select('promos.*', 'business_locations.name as lokasi', DB::raw('COALESCE((SELECT COUNT(t.promo_id) FROM transactions t WHERE t.transaction_date like "'.$date.'%" AND t.promo_id=promos.id '.($location_id == 0 ? '' : 'AND t.location_id='.$location_id ).'), 0) AS total_trx'))
                            ->whereRaw('promos.id IN (SELECT id FROM `promos` WHERE promo_start LIKE "'.$date.'%" OR promo_end LIKE "'.$date.'%")')
                            ->join('business_locations', 'business_locations.id', '=', 'promos.location_id');
            if (!empty($request->input('location_id'))) {
            ...
            $datatable = Datatables::of($data)
                ->addColumn(
                    ...
                    function ($row) {
                        $html = '' ;            
                        $html .= '<button class="btn btn-xs btn-success" data-toggle="modal" data-target="#detailUse" onclick="showDetail(this)" data-id="'.$row->id.'"><i class="fa fa-eye"></i></button>';
                        return $html;
                    }
                )
                ->make(true);
        ...
        $month=json_encode(explode('-', $date));
        return view('report.promo_report')->with(compact('month', 'location_id', 'business_locations'));
    }
    public function detailPromoUse($id){
        $data['data']=Transaction::where('promo_id', $id)->select('contacts.name', 'transaction_date')->join('contacts', 'contacts.id', '=', 'transactions.contact_id')->get();
        return $data;
    }
    public function getReportMonth(Request $request){
        ...
        // kasbon
        $query=DB::table('tbl_pegawai')
                    ->select('tbl_pegawai.nama_pegawai', DB::raw('COALESCE((SELECT SUM(tp.total) FROM tbl_pengeluaran tp WHERE tp.tipe="kasbon" AND tp.tanggal like "'.$date.'%" AND tp.id_pegawai=tbl_pegawai.id_pegawai), 0) AS total_kasbon'));
        if ($location_id != 0) {
            $query->where('tbl_pegawai.location_id', $location_id);
        }
        $kasbon=$query->get();
        ...
        for ($i=1; $i <= $jumlah_hari ; $i++) { 
            ...
            $query_pengeluaran_other=DB::table('tbl_pengeluaran_other')
                        ->select('tbl_pengeluaran_other.total', 'tbl_pengeluaran_other.tanggal', 'tbl_pengeluaran_other.deskripsi_pengeluaran', 'tbl_pengeluaran_other.notes')
            ...
            $temp_pengeluaran[$i]['tanggal']=$day;
        }
        // pengeluaran
        $query_pengeluaran=DB::table('tbl_pengeluaran')
                    ->select(DB::raw('SUM(IF(tipe="pengeluaran", tbl_pengeluaran.total, 0)) AS jumlah_pengeluaran'), DB::raw('SUM(IF(tipe="setoran", tbl_pengeluaran.total, 0)) AS jumlah_setoran'), DB::raw('SUM(IF(tipe="kasbon", tbl_pengeluaran.total, 0)) AS jumlah_kasbon'))
        ...
        $query_pengeluaran_other=DB::table('tbl_pengeluaran_other')
                    ->select(DB::raw('SUM(IF(notes="Pengeluaran Manajemen" && is_entry=true, tbl_pengeluaran_other.total, 0)) AS pengeluaran_manajemen'), DB::raw('SUM(IF(notes="Pengeluaran Manajemen" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengurangan_pengeluaran_manajemen'), DB::raw('SUM(IF(notes="Pengeluaran Sewa" && is_entry=true, tbl_pengeluaran_other.total, 0)) AS pengeluaran_sewa'), DB::raw('SUM(IF(notes="Pengeluaran Sewa" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengurangan_pengeluaran_sewa'), DB::raw('SUM(IF(notes="Tabungan THR" && is_entry=true, tbl_pengeluaran_other.total, 0)) AS tabungan_thr'), DB::raw('SUM(IF(notes="Tabungan THR" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengurangan_tabungan_thr'), DB::raw('SUM(IF(notes="Tabungan Amortisasi" && is_entry=true, tbl_pengeluaran_other.total, 0)) AS tabungan_amortisasi'), DB::raw('SUM(IF(notes="Tabungan Amortisasi" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengurangan_tabungan_amortisasi'), DB::raw('SUM(IF(notes="Deposit Pegawai" && is_entry=true, tbl_pengeluaran_other.total, 0)) AS deposit_pegawai'), DB::raw('SUM(IF(notes="Deposit Pegawai" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengurangan_deposit_pegawai'))
                    ->where('tanggal', 'like', '%'.$date.'%')
                    // ->where('tanggal', '<=', $date.'-'.$jumlah_hari)
                    ->join('users', 'users.id', '=', 'tbl_pengeluaran_other.user_id');
        if ($location_id != 0) {
            $query_pengeluaran_other->where('tbl_pengeluaran_other.location_id', $location_id);
        }
        ...
        $date2 = date('Y-m', strtotime('-1 months', strtotime($date))); 
        $newDate2=explode('-', $date2);
        $jumlah_hari2 = cal_days_in_month(CAL_GREGORIAN, $newDate2[1], $newDate2[0]);
        
        $query_pengeluaran_other_before=DB::table('tbl_pengeluaran_other')
                    ->select(DB::raw('SUM(IF(notes="Pengeluaran Manajemen" && is_entry=true, tbl_pengeluaran_other.total, 0)) - SUM(IF(notes="Pengeluaran Manajemen" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengeluaran_manajemen'), DB::raw('SUM(IF(notes="Pengeluaran Sewa" && is_entry=true, tbl_pengeluaran_other.total, 0)) - SUM(IF(notes="Pengeluaran Sewa" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS pengeluaran_sewa'), DB::raw('SUM(IF(notes="Tabungan THR" && is_entry=true, tbl_pengeluaran_other.total, 0)) - SUM(IF(notes="Tabungan THR" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS tabungan_thr'), DB::raw('SUM(IF(notes="Tabungan Amortisasi" && is_entry=true, tbl_pengeluaran_other.total, 0)) - SUM(IF(notes="Tabungan Amortisasi" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS tabungan_amortisasi'), DB::raw('SUM(IF(notes="Deposit Pegawai" && is_entry=true, tbl_pengeluaran_other.total, 0)) - SUM(IF(notes="Deposit Pegawai" && is_entry=false, tbl_pengeluaran_other.total, 0)) AS deposit_pegawai'))
                    // ->where('tanggal', 'like', '%'.$date.'%')
                    ->where('tanggal', '<=', $date2.'-'.$jumlah_hari2)
                    ->join('users', 'users.id', '=', 'tbl_pengeluaran_other.user_id');
        if ($location_id != 0) {
            $query_pengeluaran_other_before->where('tbl_pengeluaran_other.location_id', $location_id);
        }
        $pengeluaran_other_before=$query_pengeluaran_other_before->first();
        ...
        $month=json_encode(explode('-', $date));
        $query_other_gaji=DB::table('tbl_other_gaji')
                                ->join('tbl_pegawai', 'tbl_pegawai.id_pegawai', 'tbl_other_gaji.id_pegawai')
                                ->select(DB::raw('COALESCE(SUM(tbl_other_gaji.deposit_pegawai), 0) as deposit_pegawai'))
                                ->where('bulan', $date);
        if ($location_id != 0) {
            $query_other_gaji->where('tbl_pegawai.location_id', $location_id);
        }
        $other_gaji=$query_other_gaji->first();
    ...
    public function getCountTrxEmployee(Request $request){        
        ...
        if (request()->ajax()) {
            ...
            $date_range = $request->input('date_filter');
            $start_date=null;
            if (!empty($date_range)) {
                $date_range_array = explode(' - ', $date_range);
                $start_date = date('Y-m-d H:i:s', strtotime($date_range_array[0]));
                $end_date = date('Y-m-d H:i:s', strtotime($date_range_array[1]));
            }

            $pegawai=DB::table('tbl_pegawai');
            if($location_id != null){
                $pegawai->where('location_id', $location_id);
            }
            $query_pegawai=$pegawai->pluck('id_pegawai');
            $trx=DB::table('transaction_sell_lines')
                    ->join('tbl_pegawai', 'tbl_pegawai.id_pegawai', 'transaction_sell_lines.id_pegawai')
                    ->join('products', 'products.id', 'transaction_sell_lines.product_id')
                    ->whereIn('transaction_sell_lines.id_pegawai', $query_pegawai)
                    ->where('transaction_sell_lines.is_item_paket', 0)
                    ->select(DB::raw('SUM(transaction_sell_lines.quantity) as total'), DB::raw('MAX(tbl_pegawai.nama_pegawai) as nama_pegawai'), 'transaction_sell_lines.id_pegawai',  DB::raw('MAX(products.name) as nama_product'))
                    ->groupBy('transaction_sell_lines.id_pegawai', 'transaction_sell_lines.product_id');
            if ($start_date != null) {
                $trx->where('transaction_sell_lines.created_at', '>=', $start_date);
                $trx->where('transaction_sell_lines.created_at', '<=', $end_date);
            }
            $trx->get();
            $datatable = Datatables::of($trx)
                ->make(true);
...
//             $temp=array('jasa' => 0, 'non_jasa' => 0);
//             $transaction=DB::select('SELECT * FROM transactions WHERE type="sell" and status="final" and transaction_date like "'.$day.'%" '.($location_id != 0 ? 'AND transactions.location_id="'.$location_id.'"' : '').'');
//             $total_day=$total_real=0;
//             foreach ($transaction as $key => $value) {
//                 $detail_trx=DB::select('SELECT tsl.*, p.enable_stock FROM transaction_sell_lines tsl join products p on p.id = tsl.product_id WHERE transaction_id = '.$value->id.' AND tsl.is_item_paket = 0 ');
//                 $count=count($detail_trx);
//                 // $diskon=$value->discount_type == 'fixed' ? ($value->discount_amount / ($count != 0 ? $count : 1)) : 0;
//                 $discount_cut=($value->discount_type == 'fixed' ? ($value->discount_amount / ($count != 0 ? $count : 1)) : (($value->total_before_tax * ($value->discount_amount/100)) / ($count != 0 ? $count : 1)));
//                 $total_per_item=0;
//                 foreach ($detail_trx as $k => $v) {
//                     // $total_item=$value->discount_type == 'fixed' ? ($v->quantity * $v->unit_price_inc_tax) - $diskon : $v->quantity * ($v->unit_price_inc_tax - ($v->unit_price_inc_tax * ($value->discount_amount / 100)));
//                     $total_item=($v->quantity * $v->unit_price_before_discount) - $discount_cut;
//                     $temp['jasa']=$temp['jasa'] + ($v->enable_stock == 0 ? $total_item : 0);
//                     $temp['non_jasa']=$temp['non_jasa'] + ($v->enable_stock == 1 ? $total_item : 0);
//                     $total_real+=$total_item;
//                     $total_per_item+=$total_item;
//                 }
//                 echo 'total_item : '.$count.'</br>';
//                 echo 'id : '.$value->id.'</br>';
//                 echo (int)$value->final_total.' - '.$total_per_item;
//                 echo '</br>';
//                 $total_day+=$value->final_total;
//                 $total_all+=$value->final_total;
//             }
//             echo 'total dalam sehari : '.$total_day.' - '.$total_real;
//             echo '</br>';
//             $query_pengeluaran=DB::table('tbl_pengeluaran')
//                         ->select('tbl_pengeluaran.total', 'tbl_pengeluaran.tanggal', 'tbl_pengeluaran.deskripsi_pengeluaran', 'tbl_pengeluaran.notes')
//                         ->join('users', 'users.id', '=', 'tbl_pengeluaran.user_id')
