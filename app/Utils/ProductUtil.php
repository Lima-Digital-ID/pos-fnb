...
class ProductUtil extends Util
{
     ...
     * @return Obj
     */
    public function countProductTrending($product_id, $business_id, $time, $filters){
      $query = Transaction::join(
          ...
          'tsl.transaction_id'
                  ...
                  ->where('transactions.transaction_date', 'like', $time.'%')
    ...
    public function trendingProducts($business_id, $filters = []){
      $query = Transaction::join(
            'transaction_sell_lines as tsl',
            'transactions.id',
            '=',
    ...
    public function getTrendingProducts($business_id, $filters = [])
    {
        ...
        if (!empty($filters['date'])) {
            // $query->whereBetween(DB::raw('date(transaction_date)'), [$filters['start_date'],
            //     $filters['end_date']]);
            $query->where(DB::raw('date(transaction_date)'), 'like', $filters['date'].'%');
        }else{
            $query->where(DB::raw('date(transaction_date)'), 'like', date('Y-m-d').'%');
        }
    ...
    public function getTrendingProducts2($business_id, $filters = [])
    {
        $query = Transaction::join(
            'transaction_sell_lines as tsl',
            'transactions.id',
