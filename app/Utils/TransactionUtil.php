...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'id_kategori_harga' =>$input['id_kategori_harga'],
            ...
            'promo_id' => !empty($input['promo_id']) ? $input['promo_id'] : null,
        ]);
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        ...
                                        'id_pegawai' => $product['pegawai'],
                                        'is_item_paket' => $product['is_item_paket'],
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $pegawai = DB::table('tbl_pegawai')
            ->leftJoin('transaction_sell_lines', 'transaction_sell_lines.id_pegawai', '=', 'tbl_pegawai.id_pegawai')
            ->where('transaction_sell_lines.transaction_id', $transaction_id)
            ->select(DB::raw('MAX(tbl_pegawai.nama_pegawai) AS nama_pegawai'))
            ->groupBy('tbl_pegawai.id_pegawai')
            ->get();
        ...
        $output = [
            ...
            'nama_pegawai' => (count($pegawai) > 0 ? rtrim($nama_pegawai, ', ') : ""),
        ...
        if ($transaction_type == 'sell') {
            ...
            $lines = $transaction->sell_lines()->whereNull('parent_sell_line_id')->where('is_item_paket', 0)->with($sell_line_relations)->get();
        ...
        $promo=Promo::join('transactions', 'promos.id', '=', 'transactions.promo_id')->where('transactions.id', $transaction_id)->first();
        $cg=DB::table('customer_groups')->join('transactions', 'customer_groups.id', '=', 'transactions.customer_group_id')->where('transactions.id', $transaction_id)->first();
        //Discount
        $output['line_discount_label'] = $invoice_layout->discount_label;
        $output['discount_label'] = $invoice_layout->discount_label;
        $output['discount_label'] .= $promo != null ? ' '.$promo->promo_name : ($cg != null ? ' '.$cg->name : '');
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $detail_product=DB::table('product_pakets')
                             ->select('products.name AS item_name', 'product_pakets.*', 'variations.*')
                             ->join('products', 'products.id', '=', 'product_pakets.product_id')
                             ->join('variations', 'variations.product_id', '=', 'product_pakets.product_id')
                             ->where('product_item', $product->id)
                             ->get();
            $line_array = [
                //Field for 1st column
                'detail'    =>$detail_product,
                'is_paket'    =>$product->is_paket,
                'name' => $product->name,
