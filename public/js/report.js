...
function getAvgProductSell(){
    var div_detail=$('#detail_total');
    var start = '';
    var end = '';
    ...
    const firstDate = new Date(start)-oneDay;
    ...
    var data = { start_date: start_date, end_date: end_date, location_id: location_id , variation_id : variation_id, location_id : location_id};
    
    var detail='<p>Rata-rata Penjualan Produk dari tanggal '+formatDate(start)+' sampai tanggal '+formatDate(end);
    total_sell=0;
    $.ajax({
        url: '/reports/product-sell-avg-report',
        method: 'GET',
        dataType: 'json',
        data: data,
        async : false,
        success: function(data) {
            ...
            for (var i = 0; i < arrData.length; i++) {
                detail+='<p>- '+parseFloat(arrData[i]['total_qty_sold'])+' '+arrData[i]['unit']+'</p>';
                total_sell+=parseFloat(arrData[i]['total_qty_sold']);
            };
        },
    });
    avg=total_sell/diffDays;
    detail+='<p>'+parseFloat(avg).toFixed(2)+' item/hari</p>';
