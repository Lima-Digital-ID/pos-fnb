...
$(document).ready(function() {
    ...
    $(document).on('change', 'input#single_dpp', function(e) {
        ...
        $('input.harga_kategori').val(selling_price);
        __write_number($('input#single_dsp'), 0);
        ...
        $('input.harga_kategori_inc_tax').val(selling_price_inc_tax);
        __write_number($('input#single_dsp_inc_tax'), 0);
    });
    ...
    $(document).on('keyup', '.harga_kategori', function(e) {
        var tax_rate = $('select#tax')
            .find(':selected')
            .data('rate');
        tax_rate = tax_rate == undefined ? 0 : tax_rate;

        var harga_inc_tax_val
        if(tax_rate==0){
            harga_inc_tax_val = $(this).val()
        }
        else{
            harga_inc_tax_val = parseInt($(this).val()) + ($(this).val() * tax_rate / 100)
        }
        var selector = $(this).data('id')
        $(selector).val(harga_inc_tax_val)
    })
