$(document).ready(function() {
    ...
    $('#customer_id').on('select2:select', function(e) {
        ...
        $('#contact_number').val(data.mobile);
        
        // if (data.customer_group.toLowerCase() == 'member') {
        //     $('#contact_member').prop('checked', true);
        // }else{
        //     $('#contact_member').prop('checked', false);
        // }
        if (data.amount_diskon != 'null') {
            if (data.amount_diskon < 0) {
                editDiscount(Math.abs(data.amount_diskon));
            }else{
                editDiscount(0);
            }
        }else{
        ...
        today = yyyy + '-' + mm + '-' + dd;
        // console.log(data)
        if (data.exp_member_date == today && data.customer_group_id != null) {
            toastr.error('Member ini hampir kadaluarsa, harap diperpanjang');
        }
        // $('#contact_member').val(data.customer_group != null ? (data.customer_group == 'Member' ? 'Ya' : 'Tidak') : '');
    });
...
function get_saldo_all(locationId='') {
    $.ajax({
        method: 'GET',
        data : {location_id:locationId},
        url: '/sells/pos/get-saldo-all',
        dataType : 'json',
        async:false,
        success: function(result) {
            arrData=result;
...
function get_promo_list(id) {
    $.ajax({
        method: 'GET',
        url: '/sells/get-promo/'+id,
        dataType : 'json',
        async:false,
        success: function(result) {
            promo=result;
            var promo_option=$('#promo_id');
            promo_option.empty();
            promo_option.append('<option value="">Pilih Promo</option>');
            for (var i = 0; i < promo.length; i++) {
                promo_option.append('<option value="'+promo[i]['id']+'">'+promo[i]['promo_name']+'</option>');
            };
        },
    });
}
