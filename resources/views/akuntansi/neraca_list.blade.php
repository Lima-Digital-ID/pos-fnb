            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        @endphp
                        @foreach ($data['detail_saldo'] as $key => $value)
                            @if($value['data']->jumlah_debit != 0 || $value['data']->jumlah_kredit != 0)
                            @php $saldo=$value['detail'] @endphp
                            <tr>
                                <td>{{$value['data']->no_akun}}</td>
                                <td>{{$value['data']->nama_akun}}</td>
                                @php
                                if ($value['data']->id_parent == 3 || $value['data']->id_parent == 7) {
                                    $a=(($saldo != null ? $saldo->jumlah_saldo : 0) + $value['data']->jumlah_debit) - $value['data']->jumlah_kredit;
                                    if($a < 0){
                                        $total_kredit+=abs($a);
                                    }else{
                                        $total_debit+=$a;
                                ...
                                @php
                                    ...
                                    $b=(($saldo != null ? $saldo->jumlah_saldo : 0) + $value['data']->jumlah_kredit) - $value['data']->jumlah_debit;
                                    if($b < 0){
                                        $total_debit+=abs($b);
                                    }else{
                                        $total_kredit+=$b;
